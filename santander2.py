...
def processData(in_file_name, cust_dict, full_df, target_cols):
    x_vars_list = []
    y_vars_list = []
    count = 0
...
#            continue
        ...
        x_vars.append(getPrimary(row))

#        lag15 = ['2015-01-28', '2015-02-28', '2015-03-28', '2015-04-28', '2015-05-28']
#        lag16 = ['2016-01-28', '2016-02-28', '2016-03-28', '2016-04-28', '2016-05-28']

        if row['fecha_dato'][3] == '5':
            pers_services = full_df.loc[full_df.ncodpers==cust_id][:5]
            
#            x_vars += list(pers_services[target_cols].sum())

            prevs = list(pers_services[target_cols].values.flatten())
            while len(prevs) < 110:
                prevs.insert(0,0)
            x_vars += prevs

        elif row['fecha_dato'][3] == '6':
            pers_services = full_df.loc[full_df.ncodpers==cust_id][-5:]
...
if __name__ == "__main__":
    ...
    data_path = "/home/juho/python/kaggle-santander/"    
    full_df = pd.read_csv(data_path + "train_ver2.csv", usecols=['fecha_dato', 'ncodpers'] + target_cols)
    print('Services read')

    train_file =  open(data_path + "train_ver2.csv")
    x_vars_list, y_vars_list, cust_dict = processData(train_file, {}, full_df, target_cols)
    ...
    x_vars_list, y_vars_list, cust_dict = processData(test_file, cust_dict, full_df, target_cols)
