...
if __name__ == "__main__":
    ...
    print('\nStarted')
    directory = '/home/juho/python/kaggle-allstate/'
    ...
    for column in categorical_feats:
        train_col_symbols = set(train[column].unique())
        test_col_symbols = set(test[column].unique())
        if train_col_symbols != test_col_symbols:
            common_symbols = train_col_symbols.intersection(test_col_symbols)
            
            def select_common(x):
                if x not in common_symbols:
                    return None
                return x
            
            train_test[column] = train_test[column].apply(lambda x: select_common(x), 1)
    ...
    train_test["cont6"] = np.log(preprocessing.minmax_scale(train_test["cont6"]) + 0000.1) #.0001?
    ...
    print('')
    for comb in itertools.combinations(COMB_FEATURE[:4], 2): #!
        feat = comb[0] + "_" + comb[1]
        train_test[feat] = train_test[comb[0]] + train_test[comb[1]]
        train_test[feat] = train_test[feat].apply(encode)
        print('Combining Columns:', feat, end=', ')
    ...
    ss = StandardScaler()
    train_test.drop(['cat33','cat47','cat46','cat61','cat89','cat34','cat78','cat14'], inplace=True, axis=1)
    train_test[numeric_feats] = ss.fit_transform(train_test[numeric_feats].values)

    train = train_test[:ntrain].copy()
    test = train_test[ntrain:].copy()

    print('\nMedian Loss:', train.loss.median())
    print('Mean Loss:', train.loss.mean())

    ids = train_test[train_test.loss.isnull()].id
    train_y = np.log(train['loss'].astype(float) + shift)
    ...
    result = pd.DataFrame(ids.values, columns=['id'])
    result['loss'] = mpred
    print("%d-fold average prediction:" % n_folds)

    now = datetime.now()
    score = str(round((cv_sum / n_folds), 6))
    sub_file = 'allstate_' + str(score) + '_' + str(
